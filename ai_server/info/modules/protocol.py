...
class OCRGeneralRequest(BaseModel):
    ...
    det_limit_side_len: Optional[int] = Field(default=960, ge=64, le=4096, description="检测图片的长边限制")
    det_db_thresh: Optional[float] = Field(default=0.3, ge=0, le=1, description="检测阈值")
    det_db_box_thresh: Optional[float] = Field(default=0.6, ge=0, le=1, description="检测框阈值")
    det_db_unclip_ratio: Optional[float] = Field(default=2.0, ge=0.1, le=12.0, description="检测框大小")
    rec_drop_score: Optional[float] = Field(default=0.5, ge=0, le=1, description="识别过滤阈值, 取值范围：0～1")
    cls_thresh: Optional[float] = Field(default=0.9, ge=0, le=1, description="方向阈值, 取值范围：0～1")
