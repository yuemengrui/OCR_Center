...
def create_predictor(args, mode, logger):
    ...
    if args.use_mindir:
        ...
        try:
            device_type = paddle.device.get_device()
        except:
            device_type = ''

        logger.info(f"device: {device_type}")

        if args.use_gpu and 'gpu' in device_type:
            gpu_id = get_infer_gpuid()
            if gpu_id is None:
                logger.warning(
                    "GPU is not found in current device by nvidia-smi. Please check your device or ignore it if run on jetson."
                )
            logger.info(f"model load in device: GPU")
            config.enable_use_gpu(args.gpu_mem, args.gpu_id)
        elif 'npu' in device_type:
            logger.info(f"model load in device: NPU")
